...
def get_instance_details(service, project_id: str, instance_name: str) -> tuple:
    ...
    db_type      = "mysql" if "MYSQL" in db_version else "postgresql" if "POSTGRES" in db_version else "sql_server"
    ...
    with Connector(refresh_strategy="LAZY") as connector:
        with open("config.csv", newline="", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                # Step 4 — Fetch uptime from first available database (handle it)
                
                # Handling db_type

                # Step 5 — Run health check queries
                try:
                    ...
                    if auth_type == "iam":
                        engine = get_iam_engine({
                            ...
                            "user":       db_user,
                            "database":   "mysql" if db_type == "mysql" else "postgres" if db_type == "postgresql" else "sqlserver",
                            "db_type":    "mysql" if db_type == "mysql" else "postgres" if db_type == "postgresql" else "sql_server"
                        ...
                        if db_type == "postgresql":
                            engine = connect_postgresql(connector, connection_name, db_user, db_pass)
                        else:
                            engine = connect_mysql(connector, connection_name, db_user, db_pass)
                    report[instance_name]["health_checks"] = run_queries(engine, db_type, all_queries)
                    print(f"    ✅ Done.")
