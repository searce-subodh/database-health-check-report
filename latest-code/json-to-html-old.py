...
for instance, data in report_data.items():
  ...
  if specs and "Error" not in specs:
    ...
    html_content += f'<div class="error-box">[X] {specs["Error"]}</div>'

  # -- Resource Utilization --
  if utilization:
    html_content += (
        '<div class="section-title" style="margin-top:16px;">[Metrics]'
  ...
  if "connection_error" in health_checks:
    html_content += (
        f'<div class="error-box">[X] {health_checks["connection_error"]}</div>'
    )
  elif health_checks:
