...
def resolve_databases(
  ...
  if db_name_field.upper() == "ALL":
    ...
    databases = get_all_databases(
        connector, connection_name, db_user, db_pass, db_type
    )
...
def get_all_databases(
  ...
  if db_type == "postgresql":
    engine = connect_postgresql(
        connector, connection_name, db_user, db_pass, "postgres"
    )
    with engine.connect() as conn:
      result = conn.execute(sqlalchemy.text("""
                ...
                ORDER BY datname;
            """))
      databases = [r[0] for r in result.fetchall()]

  elif db_type == "mysql":
    engine = connect_mysql(
        connector, connection_name, db_user, db_pass, "mysql"
    )
    with engine.connect() as conn:
      result = conn.execute(sqlalchemy.text("""
  ...
  with Connector(refresh_strategy="LAZY") as connector:
    with open("config.csv", newline="", encoding="utf-8") as csvfile:
      ...
      for row in reader:
        ...
        specs, connection_name, db_type = get_instance_details(
            sqladmin, project_id, instance_name
        )
        if isinstance(specs, dict) and "Error" in specs:
          report[instance_name]["provisioned_specs"] = specs
          print("  ❌ Skipping: could not fetch instance details.")
          continue

        report[instance_name]["provisioned_specs"] = specs

        if not connection_name:
          print("  ❌ Skipping: valid connection_name not found.")
          continue
